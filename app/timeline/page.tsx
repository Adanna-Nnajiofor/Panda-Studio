...
export default function TimelinePage() {
  ...
  useEffect(() => {
    apiJson<{ projects: Project[] }>("/projects")
      .then((d) => {
        ...
        if (d.projects?.length) setSelected(d.projects[0]._id);
      })
      .catch((e) => setError(getErrorMessage(e, "Failed to load projects.")))
  ...
  const milestoneStatus = (m: Milestone) => {
    if (m.status === "completed") return "completed" as const;
    if (new Date(m.dueDate) < new Date()) return "overdue" as const;
  ...
  return (
    <DashboardShell
      kicker="Projects"
      title="Timeline"
      summary="Track project milestones, deadlines, and production progress."
    >
      {loading ? (
          ...
          <div className="space-y-2">
            ...
            {projects.map((p) => (
              <button
                ...
                className={`w-full border-4 border-black p-3 text-left shadow-[4px_4px_0_0_#000] transition-transform hover:-translate-y-0.5 ${
                  selected === p._id ? "bg-black text-[#f2eadf]" : "bg-white"
                }`}
              >
                <p className="text-xs font-black uppercase">{p.title}</p>
                <span className="mt-1 inline-block border border-current px-2 py-0.5 text-[0.6rem] font-black uppercase">
            ...
            <div className="border-4 border-black bg-white p-5 shadow-[8px_8px_0_0_#000]">
                ...
                <div>
                  ...
                  {active.client ? (
                    <p className="text-sm text-gray-600">
                      Client: {active.client.fullName}
                    </p>
                  ) : null}
                </div>
                <span
                  className={`border-2 border-black px-3 py-1 text-xs font-black uppercase ${
                    STATUS_COLOR[active.progressStatus] ?? ""
                  }`}
                >
                  {active.progressStatus.replace("_", " ")}
                </span>
              </div>
              ...
              {active.startDate || active.endDate ? (
                <div className="mt-4 flex gap-6 text-xs font-black">
                  {active.startDate ? (
                    <span>
                      Start: {new Date(active.startDate).toLocaleDateString()}
                    </span>
                  ) : null}
                  {active.endDate ? (
                    <span>
                      End: {new Date(active.endDate).toLocaleDateString()}
                    </span>
                  ) : null}
                </div>
              ) : null}
                  ...
                  <div className="relative space-y-4 pl-6">
                    ...
                    {active.milestones.map((m) => {
                      ...
                      return (
                        <div
                          key={m._id}
                          className={`relative border-2 p-4 ${MILESTONE_COLOR[ms]}`}
                        >
                          ...
                          <div className="flex items-start justify-between gap-2">
                            ...
                            <span className="border border-current px-2 py-0.5 text-[0.6rem] font-black uppercase">
                              {ms}
                            </span>
                          </div>
