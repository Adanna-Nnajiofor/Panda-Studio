...
const isUserAllowedForProject = (
  ...
  if (user.role === "client") {
    return (
      String(project.client?._id ?? project.client) === user.id ||
      String(project.createdBy?._id ?? project.createdBy) === user.id
    );
  }

  if (isOperationalRole(user.role)) {
    const assignedCrew = Array.isArray(project.assignedCrew)
      ? project.assignedCrew.map(String)
      : [];
    const assignedStaff = Array.isArray(project.assignedStaff)
      ? project.assignedStaff.map(String)
      : [];
    const assignedUsers = Array.isArray(project.assignedUsers)
      ? project.assignedUsers.map(String)
      : [];
    return [...assignedCrew, ...assignedStaff, ...assignedUsers].includes(
      user.id,
    );
  }
...
export const getPublicProjects = async (
  ...
  const projects = await Project.find({ isArchived: { $ne: true } })
    .select("_id progressStatus expiryDate isArchived createdAt updatedAt")
    .sort({ createdAt: -1 })
    .limit(100);
...
export const getMyProjects = async (
  ...
  const projects = await Project.find(buildVisibilityFilter(authUser)).sort({
    createdAt: -1,
  });
...
export const createProject = async (
  ...
  try {
    ...
    const { booking, client, expiryDate, progressStatus, isArchived } =
      req.body as {
        booking?: string;
        client?: string;
        expiryDate?: string;
  ...
  try {
    ...
    return res.status(500).json({
      ...
      message:
        error instanceof Error
          ? error.message
          : "Failed to assign project members",
    });
  }
};
