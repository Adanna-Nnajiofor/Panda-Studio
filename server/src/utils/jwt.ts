...
const getJwtSecret = (): string => {
  ...
  if (typeof secret === "string" && secret.trim().length > 0) {
    ...
    if (
      ...
      throw new Error(
        `[jwt] JWT_SECRET must be at least ${MIN_SECRET_LENGTH} characters in production.`,
      );
    }
  ...
  logger.warn(
    "[jwt] Using weak default JWT_SECRET in development. Set JWT_SECRET in .env for production.",
  );
